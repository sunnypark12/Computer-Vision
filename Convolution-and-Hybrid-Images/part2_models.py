...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernel2d = create_Gaussian_kernel_2D(cutoff_frequency)
        kernel = torch.Tensor(kernel2d).unsqueeze(0).unsqueeze(0)
    ...
    def forward(
        ...
        #for all index in size length
        for idx in range(image1.size(0)):
            # Get kernel
            kernel = self.get_kernel(cutoff_frequency[idx].item())

            # Get low frequency from img1
            low_frequencies = self.low_pass(image1[idx].unsqueeze(0), kernel)
            low_frequencies_list.append(low_frequencies)

            # Get high frequency from img2
            low_frequencies_image2 = self.low_pass(image2[idx].unsqueeze(0), kernel)
            high_frequencies = image2[idx].unsqueeze(0) -low_frequencies_image2
            high_frequencies_list.append(high_frequencies)

            # Create hybrid image and append as well
            hybrid_image = torch.clamp(low_frequencies + high_frequencies, 0, 1)
            hybrid_images_list.append(hybrid_image)
