...
def match_features_ratio_test(
    ...
    samllest = sorted_dists[:, :2]
    smallestD = dists[np.arange(dists.shape[0]), samllest[:, 0]]
    smallestD2 = dists[np.arange(dists.shape[0]), samllest[:, 1]]
    
    threshold = 0.8
    standard = (smallestD / (smallestD2 + 1e-10)) < threshold
    
    matches = np.column_stack((np.arange(dists.shape[0])[standard], samllest[standard, 0]))
    
    confidences = (1 / (smallestD[standard] + 1e-10))
    sorted_confidents = np.argsort(-confidences)


    matches = matches[sorted_confidents, :]
    confidences = confidences[sorted_confidents]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
