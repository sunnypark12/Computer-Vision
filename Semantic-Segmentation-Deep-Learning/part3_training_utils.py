...
def get_model_and_optimizer(args) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    if args.arch == "SimpleSegmentationNet":
        ...
        if model.ppm is not None:
            param_groups.append({"params": model.ppm.parameters(), "lr": args.base_lr * 10})
        if hasattr(model, "aux") and model.aux is not None:
            param_groups.append({"params":model.aux.parameters(), "lr": args.base_lr * 10})
        
    else: 
        raise ValueError(f"Unknown architecture: {args.arch}")
    
    optimizer = torch.optim.SGD(
        param_groups, lr=args.base_lr, momentum=args.momentum, weight_decay=args.weight_decay
    )
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
