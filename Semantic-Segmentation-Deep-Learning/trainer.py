        ...
        if args.evaluate:
            with torch.no_grad():
                loss_val, mIoU_val, mAcc_val, allAcc_val = run_epoch(
                    args, device, val_loader, model, optimizer=None, epoch=epoch, split="val"
                )
...
def transfer_train(args, device: torch.device):
    """ """
    # model, optimizer = get_model_and_optimizer(args)
    args.classes = 11
    pre_model = load_pretrained_model(args, device)
    args.classes = 2
